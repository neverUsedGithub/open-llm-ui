...
The file system of the python environment is NOT persistent, it will reset between code_interpreter calls.`,
  ...
  },
  {
    ...
    async execute(properties: { url: string; query?: string }, ctx) {
      try {
...
const inputTags: InputTag[] = [
...

  {
    ...
    prompt:
      "You should think through your next message even if the user's request seems simple or straightforward. Consider all possible edge-cases, cover every possible case the user might be asking you.",
...
function SubMessageView(props: { subMessage: SubChatMessage; messageState: ChatMessageState; latest: boolean }) {
  ...
  if (props.subMessage.thinking) {
    ...
    function thinkingIncrement() {
      if (!props.latest || props.messageState !== "thinking") return;
      ...
      <div class="text-foreground-muted flex flex-col gap-2">
        <button class="flex cursor-pointer items-center gap-2" onClick={() => setThinkingExpanded((prev) => !prev)}>
          ...
          <span class="-translate-y-0.25">
            {props.latest ? "Thinking" : "Thought"} for {(thinkingTime() / 1000).toFixed(1)}s
          </span>
        </button>
...
function createSubMessage(data: SubChatMessageData): SubChatMessage {
  ...
  return { kind: "text", content, stream, removeToolCall: noop, thinking: data.thinking };
}

function createChatMessage(role: "user", content: string, files: UserFile[]): UserChatMessage;
function createChatMessage(role: "assistant", content: string): AssistantChatMessage;
function createChatMessage(role: string, content: string, files?: UserFile[]): UserChatMessage | AssistantChatMessage {
  if (role === "user") return { role, content, files: files! };
...
function App() {
  ...
  let vectorDatabase: vectordb.VectorDB | null = null;
  const modelMessages: ModelChatMessage[] = [];
  ...
  async function sendMessage(userMessage: string): Promise<void> {
    ...
    vectorDatabase ??= await vectordb.createDatabase();
    ...
    modelMessages[0].content = buildSystemPrompt({ tools: promptTools });

    if (currentTag && currentTag.prompt) {
      modelMessages.push({ role: "user", content: currentTag.prompt });
    }
    ...
    setMessages([...messages(), userChatMessage, assistantMessage]);
...
\`\`\``,
      ...
      modelMessages.push({ role: "assistant", content: textContent, thinking: thinkingContent });
    } while (newTurn);
      ...
      <div class="flex w-8/12 flex-1 flex-col gap-8">
        <Show when={messages().length === 0}>
          <div class="flex h-1/2 items-end justify-center gap-4">
            ...
            <h2 class="font-handwriting -translate-y-1 text-4xl">{selectedModel()}</h2>
          </div>
        </Show>
